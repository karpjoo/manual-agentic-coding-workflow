...
Define MVP validation scope, risk-based priorities, test levels, fixture/environment strategy, and requirement-to-validation mapping.

Core question:

```text
Which MVP requirements and acceptance criteria must be validated, at what test level, with what priority and risk rationale?
```

## 2. When to use it

Use this sub-SKILL as step 1 in the Stage 8 sequence.

Previous sub-SKILL:

```text
Parent stage entrypoint / approved Stage 7 artifacts
```
...
- `/workflow/08_test_strategy/08_test_strategy.md` — Validation strategy and requirement-to-validation method mapping
...
7. Continue to `08b_acceptance_test_design` only after the output is coherent enough for the next sub-SKILL.

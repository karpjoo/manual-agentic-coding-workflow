...
Translate logical schema into physical schema when storage technology is approved and define justified index recommendations.

This is a reusable skill support guide. It is not a project artifact and it does not replace the executable `SKILL.md`.

## 2. When to use it

Use this sub-skill as part of Stage 6 after the previous required Stage 6 context is available. It answers: Given the logical schema and query patterns, what concrete storage structures and indexes are justified by approved architecture decisions and actual access patterns?
...
- `/skills/06_data_design/06b_logical_schema_query_patterns/SKILL.md` — Read the previous sub-skill procedure or use it to understand expected upstream Stage 6 draft artifacts.
...
- `/workflow/06_data/06_physical_schema.md` — Conditional: Concrete tables, collections, stores, references, constraints, and transaction/consistency boundaries.
- `/workflow/06_data/06_indexes.md` — Conditional: Required MVP indexes, candidate indexes, uniqueness/constraint indexes, and query-to-index mapping.
...
- storage technology conflicts
- deferred physical alternatives
...
- Previous stage or sub-skill: `/skills/06_data_design/06b_logical_schema_query_patterns/SKILL.md`
- Next stage or sub-skill: `06d Security, Retention, and Migration`
